...
class FORESTDataset:
    ...
    def __init__(
        self,
        path: PathLike,
        name: Literal["douban", "memetracker", "twitter"],
        *,
        download: bool = True,
    ) -> None:
    ...
    @staticmethod
    def download(
        path: PathLike, name: Literal["douban", "memetracker", "twitter"]
    ) -> None:
...
BASE = "https://raw.githubusercontent.com/albertyang33/FOREST/master/data"
SPLIT_MAPPING = {"train": "", "val": "valid", "test": "test"}
...
def _download_cascades(name: str, split: str) -> list[list[tuple[int, float]]]:
    ...
    url = f"{BASE}/{name}/cascade{split}.txt"
    ...
    for line in data.splitlines():
        ...
        chunks = line.split()
        cascade: list[tuple[int, float]] = []
        for chunk in chunks:
            node, ts = chunk.split(",")
            node = int(node)
            ts = float(ts)
            cascade.append((node, ts))
