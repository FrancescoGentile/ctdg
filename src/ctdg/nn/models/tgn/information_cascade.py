...
"""Module for the TGN model for the information cascade task."""
...
from ._model import TGN as _TGN


class TGN(LightningModule):
    """The TGN model for the information cascade task."""
    ...
    def forward(self, events: Events) -> Tensor:
        ...
        embeds = self.model.compute_node_embeddings(events.dst_nodes, events.timestamps)
        pred = self.predictor(embeds).squeeze(-1)
...
def train(config: dict[str, Any], mode: Literal["micro", "macro"]) -> None:
    """Trains the TGN model for the information cascade task."""
    if "seed" in config:
...
def test(config: dict[str, Any], mode: Literal["micro", "macro"]) -> None:
    """Tests the TGN model for the information cascade task."""
    if "seed" in config:
        utils.seed_all(config["seed"])
